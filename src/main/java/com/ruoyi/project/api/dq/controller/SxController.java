...
import com.ruoyi.project.api.dq.util.SxUtils;
 ...
 */
@Controller
@RequestMapping("/api/dq/sx")
public class SxController extends BaseController
{
	private static final Logger log = LoggerFactory.getLogger(SxController.class);
    ...
    public AjaxResult sendCode(String mobile, String originUrl, HttpServletRequest request)
    {
    	...
    	try {
			ProductOrdersLog productOrdersLog = SxUtils.reqCode(mobile);
    ...
    public AjaxResult saveOrders(String mobile, String originUrl, String validCode, HttpServletRequest request)
    {
    	...
    	try {
			...
			if(flag) {	// 校验订单
				productOrdersLog = SxUtils.validProduct(mobile, ordersNo, productId, validCode);
				productOrdersLog.setMobile(mobile);
				productOrdersLog.setOriginIp(originIp);
				productOrdersLog.setOriginUrl(originUrl);
				productOrdersLog.setUuid(ToolUtils.getUUID());
			...
			if(flag) {	// 业务办理
				productOrdersLog = SxUtils.handle(mobile, ordersNo, productId, validCode);
				productOrdersLog.setMobile(mobile);
				productOrdersLog.setOriginIp(originIp);
				productOrdersLog.setOriginUrl(originUrl);
				productOrdersLog.setUuid(ToolUtils.getUUID());
	    	...
	    	productFlowpacketOrders.setProductProvice("SX");
	    	...
	    	if(flag) {
				...
				return AjaxResult.success(200, "恭喜您，超值流量包业务办理成功");
			} else {
				// 办理失败
